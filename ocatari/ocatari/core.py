...
class OCAtari:
    ...
    def render(self):
        ...
        if not self.render_oc_overlay:
            ...
            if self.render_mode == "human":
                frameskip = self._env._frameskip if isinstance(self._env._frameskip, int) else 1
    ...
    def set_ram(self, target_ram_position, new_value):
        ...
        :type new_value: int
        """
        return self._env.ale.setRAM(target_ram_position, new_value)
    ...
    def _get_obs(self):
        return self._env.env.env._get_obs()
